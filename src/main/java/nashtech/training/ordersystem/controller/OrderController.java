...
import nashtech.training.ordersystem.service.impl.OrderServiceImpl;
...
public class OrderController {
    ...
    public ResponseEntity<List<OrderResponseDTO>> getOrders(Authentication auth){
        String username = auth.getName();
        RoleName roleName = RoleName.valueOf("ROLE_" + auth.getAuthorities().stream().findFirst().get().getAuthority().replace("ROLE_", ""));
        Role userRole = new Role(roleName);
        List<OrderResponseDTO> orders = orderService.getOrdersForUser(username, userRole);
